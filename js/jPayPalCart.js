...
(function ($) {
            ...
            return this.each(function () {
                ...
                for (var i = 0; i < itemcount; i++) {
                    if (this.settings.cartItems[i]['code'] == code) {
                        this.settings.cartItems[i]['quantity'] = parseInt(this.settings.cartItems[i]['quantity']) + 0;
                        isadded = true;
                    }
                }
                if (!isadded) {
    ...
    function PayPalCartdisplay(theDiv) {
        ...
        for (var i = 0; i < theDiv.settings.cartItems.length; i++) {
            ...
            theform += '<tr>\r\n<td><a href="javascript:void(0)" onclick="jQuery(\'#' + theDiv.id + '\').PayPalCart(\'remove\', \'' + theDiv.settings.cartItems[i]['code'] + '\');">x</a>\r\n';
            ...
            if (theDiv.settings.quantityupdate) {
                theform += '<td align="center"><input type="text" size="2" name="' + theDiv.settings.cartItems[i]['code'] + '" value="' + theDiv.settings.cartItems[i]['quantity'] + '" onblur="jQuery(\'#' + theDiv.id + '\').PayPalCart(\'update\',\'' + theDiv.settings.cartItems[i]['code'] + '\', jQuery(this).val());" /></td>\r\n';
            } else {
        ...
        //theform += '<tr class="PayPalCartTotals"><td colspan="2" align="left">Subtotal</td><td align="right">' + theDiv.settings.currencysign + parseFloat(overalltotal).toFixed(2) + '</td></tr>\r\n';
        ...
        jQuery(theDiv).html(theform).ready(function () {
            jQuery(this).find('input').keypress(function (e) {
                if (e.which == 13) {
                    var thisitem = jQuery(this).attr("name");
                    var thisval = jQuery(this).val();
                    jQuery(theDiv).PayPalCart('update', thisitem, thisval);
                    return false;
                }
            });
        });
        ...
        if (theDiv.settings.minicartid != "") {
            ...
            jQuery('#' + theDiv.settings.minicartid).html(minicart);
        }
    }
            ...
            for (i = 1; i <= cartcount; i++) {
                ...
                if (thisline != null && thisline != "") {
                    ...
                    for (j = 0; j < ARRline.length; j++) {
                        ...
                        if (keyname == 'code') {
                            jQuery('#music_seller_' + keyval).addClass('music_seller_in_cart');
                        }
