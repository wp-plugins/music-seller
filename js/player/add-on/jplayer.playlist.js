...
(function($, undefined) {
	...
	jPlayerPlaylist.prototype = {
		...
		_createListItem: function(media) {
			...
			if(media.free) {
				...
				$.each(media, function(property,value) {
					if($.jPlayer.prototype.format[property]) { // Check property is a media format.
						...
						listItem += "<a target=\"blank\" class='' href='" + value + "' tabindex='1'>" + property + "</a>";
					}
				});
				listItem += ")</span>";
			}
			if(media.price) {
				var first = true;
				listItem += "<span id=\"music_seller_" + media.key + "\" data-key='" + media.key + "' data-price='" + media.price + "' data-title='" + media.title + "'  class='music_seller_add_to_cart'><i class='music_seller_cart_icon'></i>";
				listItem += media.symbol
				listItem += media.price
				listItem += "</span>";
			}
